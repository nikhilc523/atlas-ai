...
export async function generate(input: string, projectId: string) {
    ...
    // First, let's check if ANY files exist for this project
    const allFiles = await db.sourceCodeEmbedding.findMany({
        where: { projectId },
        select: { fileName: true, id: true }
    });

    console.log(`Total files in database for this project: ${allFiles.length}`);
    ...
    console.log(`Found ${result.length} relevant files for query: "${input}"`);
    console.log(`Similarity scores:`, result.map(r => ({ file: r.fileName, similarity: r.similarity })));
